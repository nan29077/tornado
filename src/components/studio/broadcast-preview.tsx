...
const FALLBACK_BOX: Record<LayoutTarget, { w: number; h: number; cx: number; cy: number }> = {
  /*
    후원 알림: **배경 없는 큰 글씨**로 바뀌면서 차지하는 자리가 크게 넓어졌다.
    (본문 64px 두 줄 + 메시지 46px 세 줄 + 캐릭터 200px)
    옛 값(가로 46% · 세로 17%)은 작은 카드 시절의 것이라 그대로 두면 조정 상자가
    실제보다 훨씬 작게 그려져 끌 때 손에서 미끄러진다. 기본 자리는 아래쪽 가운데.
  */
  donation: { w: 0.64, h: 0.3, cx: 0.5, cy: 0.68 },
  // 게임 화면: 가운데 큰 카드.
